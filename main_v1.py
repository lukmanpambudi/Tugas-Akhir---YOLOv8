...
def getLane(results, display=2):
    ...
    mask_np = utils_v1.masking(results)
    if mask_np is not None:
        last_mask_time = time.time()  
        points = utils_v1.valTrackbars()
        hT, wT, c = frame.shape
        
        frameWarp = utils_v1.warping(mask_np, points, wT, hT)
        ...
        frameWarpPoints = utils_v1.drawPoints(frameCopy, points)
        processedImage = utils_v1.isEndOfLane(results, model, frameWarp, display=True)
        ...
        if display == 1:
            stack_img = utils_v1.stackImages(0.7, ([tampilResults, frameWarpPoints], [mask_np, frameWarp]))
            ...
            stack_img = utils_v1.stackImages(0.7, ([frameWarpPoints, mask_np], [frameWarp, processedImage]))
            cv2.imshow('Stacked Image', stack_img)
...
if __name__ == '__main__':
    ...
    utils_v1.initializeTrackbars(initializeTrackBarVals)
    frameCounter = 0
    while True:
        frameCounter += 1
        ...
        frame = utils_v1.tambahkan_bingkai(video)
